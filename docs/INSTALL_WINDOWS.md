...
| Ultralytics tente un auto-install de `onnx`/`onnxruntime` et plante avec `WinError 5: Access is denied` sur `onnxruntime_providers_shared.dll` | Encore l'ancien export via Ultralytics | Fais `git pull`. Le code exporte maintenant via `torch.onnx.export` directement (plus de DLL lock). Si tu es bloqué : ferme PowerShell, rouvre-le, fais `pip install --force-reinstall onnx` (sans toucher onnxruntime), puis relance. |
